...
from src.global_settings import VECTOR_DB_PATH, EMBEDDING_MODEL_FILE, LLM_MODEL_FILE
...
class LocalLLM:
    ...
    def __call__(self, prompt: str, stop: Optional[List[str]] = None, **kwargs) -> str:
        response = self.llm(prompt, max_new_tokens=1024, temperature=0.01, stop=stop)
        return response
    ...
    def _identifying_params(self) -> Mapping[str, Any]:
        return {"model_file": LLM_MODEL_FILE}
...
class ConversationEngine:
    ...
    def __init__(self):
        print(f"Đang tải mô hình từ: {LLM_MODEL_FILE}")
        self.llm = LocalLLM()
        print("Đã tải mô hình LLM")
        self.db = self.load_vector_db()
